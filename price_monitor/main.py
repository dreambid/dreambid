...
# SSG WAF 차단 페이지가 연속 이 횟수만큼 감지되면 수동확인으로 전환 (그 전까지는 unknown
# 유지하며 매 사이클 계속 재시도 — 쿠팡과 달리 SSG는 일시적 IP 차단이라 스크래핑 자체를
# 영구히 포기하지 않는다)
SSG_BLOCK_THRESHOLD = 3
...
async def _check_one_product_impl(product: dict) -> None:
    ...
    try:
        ...
        async with _PRODUCTS_LOCK:
            update_product_state(product_id, None, "manual_check", unknown_count=0, ssg_block_count=0)
    ...
    if result.get("verification_needed"):
        ...
        async with _PRODUCTS_LOCK:
            update_product_state(product_id, None, "점검필요", unknown_count=0, ssg_block_count=0)
        print(f"    [점검필요] {name}: {site} 확인 화면 감지({hint}) — 창을 열어뒀으니 직접 확인 필요")
        notify_manual_review(name, site, category)
        return

    # SSG WAF 차단 페이지 감지: 연속 SSG_BLOCK_THRESHOLD회 도달해야 수동확인으로 전환.
    # 그 전까지는 unknown 상태를 유지하며 다음 사이클에도 계속 재시도한다 — 쿠팡의
    # _is_manual_check()처럼 도메인 자체를 영구 스킵하는 게 아니라, 일시적 IP 차단이
    # 풀리면 자동으로 정상 수집이 재개되도록 하기 위함.
    if result.get("blocked"):
        new_block_count = product.get("ssg_block_count", 0) + 1
        new_status = "manual_check" if new_block_count >= SSG_BLOCK_THRESHOLD else "unknown"
        async with _PRODUCTS_LOCK:
            update_product_state(product_id, None, new_status, unknown_count=0, ssg_block_count=new_block_count)
        suffix = " — 수동확인 전환" if new_status == "manual_check" else ""
        print(f"    [SSG차단감지] {name}: 차단 페이지 감지 (연속 {new_block_count}회){suffix}")
        return
    ...
    if result.get("discontinued"):
        async with _PRODUCTS_LOCK:
            update_product_state(product_id, None, "discontinued", unknown_count=0, ssg_block_count=0)
    ...
    if result.get("uncertain"):
        ...
        async with _PRODUCTS_LOCK:
            update_product_state(
                ...
                unknown_count=new_unknown_count, ssg_block_count=0,
            )
